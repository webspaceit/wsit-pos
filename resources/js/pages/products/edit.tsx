...
export default function ProductEdit({ product, categories, brands, units }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        max_stock: product.max_stock, description: product.description ?? '',
        is_active: product.is_active,
    });

    const submit = (e: React.FormEvent) => { e.preventDefault(); put(`/products/${product.id}`); };
            ...
            <div className="p-4 max-w-2xl">
                ...
                <form onSubmit={submit} className="space-y-4 rounded-xl border p-6">
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit" disabled={processing} className="px-4 py-2 rounded bg-blue-600 text-white text-sm hover:bg-blue-700 disabled:opacity-50">{processing ? 'Saving...' : 'Update Product'}</button>
                    </div>
                </form>
            </div>
        </AppLayout>
...
function Field({ label, value, onChange, error, type = 'text' }: { label: string; value: any; onChange: (v: any) => void; error?: string; type?: string }) {
    return <div><label className="block text-sm font-medium mb-1">{label}</label><input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" />{error && <p className="text-xs text-red-600 mt-1">{error}</p>}</div>;
}
