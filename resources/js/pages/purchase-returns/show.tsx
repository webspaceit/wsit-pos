...
interface Return { id: number; reference_no: string; date: string; grand_total: number; payment_method: string; status: string; reason: string; notes: string; purchase: { reference_no: string }; supplier: { name: string } | null; user: { name: string }; items: Array<{ product: { name: string }; quantity: number; unit_price: number; tax_amount: number; total: number }>; }
interface Props { return_data: Return; }

export default function PurchaseReturnShow({ return_data: r }: Props) {
    return (
        <AppLayout breadcrumbs={[{ title: 'Purchase Returns', href: '/purchase-returns' }, { title: r.reference_no, href: `/purchase-returns/${r.id}` }]}>
            <Head title={`Purchase Return ${r.reference_no}`} />
            <div className="p-4 max-w-3xl space-y-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-lg font-semibold">Purchase Return — {r.reference_no}</h1>
                    <button onClick={() => router.get('/purchase-returns')} className="rounded-md border px-4 py-2 text-sm">Back</button>
                </div>
                <div className="grid grid-cols-2 gap-4 rounded-xl border p-4 text-sm">
                    <div><span className="text-muted-foreground">Purchase:</span> {r.purchase.reference_no}</div>
                    <div><span className="text-muted-foreground">Date:</span> {formatDate(r.date)}</div>
                    <div><span className="text-muted-foreground">Supplier:</span> {r.supplier?.name || '-'}</div>
