...
export default function MonthlySales({ daily, totals, month }: Props) {
    const [m, setM] = useState(month);

    return (
        <AppLayout breadcrumbs={[{ title: 'Reports', href: '/reports' }, { title: 'Monthly Sales', href: '/reports/monthly-sales' }]}>
            <Head title="Monthly Sales Report" />
            <div className="p-4 space-y-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold">Monthly Sales — {month}</h2>
                    <div className="flex items-center gap-3">
                        <input type="month" value={m} onChange={(e) => setM(e.target.value)} className="rounded-md border px-3 py-2 text-sm" />
                        <button onClick={() => router.get('/reports/monthly-sales', { month: m }, { preserveState: true })} className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white">Filter</button>
                    </div>
                </div>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    {[
                        { label: 'Total Sales', value: totals.count },
                        { label: 'Revenue', value: formatCurrency(totals.total) },
                ...
                <div className="overflow-x-auto rounded-xl border">
                    <table className="w-full text-sm">
                        <thead className="bg-muted"><tr>
                            <th className="px-3 py-2 text-left">Date</th><th className="px-3 py-2 text-right">Sales</th>
                            <th className="px-3 py-2 text-right">Revenue</th><th className="px-3 py-2 text-right">Paid</th>
