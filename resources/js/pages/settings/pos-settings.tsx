...
function BarcodeTab({ defaults }: { defaults: Record<string, string> }) {
    ...
    const save = () => post('/settings/pos-settings');

    return (
        <div className="rounded-xl border p-6 space-y-4">
            <h2 className="font-semibold">Barcode Settings</h2>
            ...
            <div className="flex justify-end"><button onClick={save} disabled={processing} className="rounded-md bg-brand px-6 py-2 text-sm text-white hover:bg-brand-dark disabled:opacity-50">{processing ? 'Saving...' : 'Save'}</button></div>
        </div>
    );
}
...
function InvoiceTab({ defaults }: { defaults: Record<string, string> }) {
    ...
    return (
        <div className="rounded-xl border p-6 space-y-4">
            ...
            <div><label className="block text-sm font-medium mb-1">Header Text</label><textarea value={data.header_text} onChange={(e) => setData('header_text', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" rows={2} /></div>
            <div><label className="block text-sm font-medium mb-1">Terms & Conditions</label><textarea value={data.terms} onChange={(e) => setData('terms', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" rows={3} /></div>
            <div className="flex gap-4">
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.show_tax} onChange={(e) => setData('show_tax', e.target.checked)} /> Show Tax</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.show_discount} onChange={(e) => setData('show_discount', e.target.checked)} /> Show Discount</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.show_customer} onChange={(e) => setData('show_customer', e.target.checked)} /> Show Customer</label>
            </div>
            <div className="flex justify-end"><button onClick={save} disabled={processing} className="rounded-md bg-brand px-6 py-2 text-sm text-white hover:bg-brand-dark disabled:opacity-50">{processing ? 'Saving...' : 'Save'}</button></div>
        </div>
    );
...
function PosTab({ defaults }: { defaults: Record<string, string> }) {
    ...
    return (
        <div className="rounded-xl border p-6 space-y-4">
            <h2 className="font-semibold">POS Terminal Settings</h2>
            <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium mb-1">Default Payment Method</label>
                    <select value={data.default_payment_method} onChange={(e) => setData('default_payment_method', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm">
                        {['cash', 'card', 'bkash', 'nagad', 'rocket'].map((m) => <option key={m} value={m}>{m.charAt(0).toUpperCase() + m.slice(1)}</option>)}
                    </select>
                </div>
                <div><label className="block text-sm font-medium mb-1">Receipt Width</label>
                    <select value={data.receipt_width} onChange={(e) => setData('receipt_width', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm">
                        <option value="58">58mm</option><option value="80">80mm</option>
                    </select>
                </div>
                <div><label className="block text-sm font-medium mb-1">Theme</label>
                    <select value={data.theme} onChange={(e) => setData('theme', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm">
                        {['light', 'dark', 'auto'].map((t) => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
                    </select>
                </div>
            </div>
            <div className="flex gap-4 flex-wrap">
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.auto_print} onChange={(e) => setData('auto_print', e.target.checked)} /> Auto Print Receipt</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.show_stock} onChange={(e) => setData('show_stock', e.target.checked)} /> Show Stock on Terminal</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.allow_discount} onChange={(e) => setData('allow_discount', e.target.checked)} /> Allow Discount</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.allow_price_edit} onChange={(e) => setData('allow_price_edit', e.target.checked)} /> Allow Price Edit</label>
            </div>
            <div className="flex justify-end"><button onClick={save} disabled={processing} className="rounded-md bg-brand px-6 py-2 text-sm text-white hover:bg-brand-dark disabled:opacity-50">{processing ? 'Saving...' : 'Save'}</button></div>
        </div>
    );
...
function MailTab({ defaults }: { defaults: Record<string, string> }) {
    ...
    return (
        <div className="rounded-xl border p-6 space-y-4">
            <h2 className="font-semibold">Mail (SMTP) Settings</h2>
            <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium mb-1">Encryption</label>
                    <select value={data.smtp_encryption} onChange={(e) => setData('smtp_encryption', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm">
...
function SmsTab({ defaults }: { defaults: Record<string, string> }) {
    ...
    return (
        <div className="rounded-xl border p-6 space-y-4">
            <h2 className="font-semibold">SMS Settings</h2>
            <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium mb-1">Provider</label>
                    <select value={data.provider} onChange={(e) => setData('provider', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm">
            ...
            <div><label className="block text-sm font-medium mb-1">Sale Template</label><textarea value={data.template_sale} onChange={(e) => setData('template_sale', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" rows={3} placeholder="Use {name}, {invoice_no}, {total}, {date} as placeholders" /></div>
            <div><label className="block text-sm font-medium mb-1">Due Reminder Template</label><textarea value={data.template_due} onChange={(e) => setData('template_due', e.target.value)} className="w-full rounded-md border px-3 py-2 text-sm" rows={3} placeholder="Use {name}, {due_amount}, {phone} as placeholders" /></div>
            <div className="flex justify-end"><button onClick={save} disabled={processing} className="rounded-md bg-brand px-6 py-2 text-sm text-white hover:bg-brand-dark disabled:opacity-50">{processing ? 'Saving...' : 'Save'}</button></div>
        </div>
    );
}
